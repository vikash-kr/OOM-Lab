...
class TestClass {
    ...
    public static void main(String args[] ) throws Exception {
      ...
      for(i=0; i<t; i++)
      {
        ...
        //Batch b = new Batch(TotalStudent);
        for(j=0; j<TotalStudent; j++){
            String s1= s.next();
            String s2= s.next();
            b.FillStudent(s1, s2, j);
            //System.out.println(j + "solution input taken"+ s1 + s2);
        }
        for(j=0; j<TotalStudent; j++){
        TestClass Test = new TestClass();
        Test.TotalAssignment = s.nextInt();
        System.out.println(b.St[j].RollNumber + " "+ b.St[j].StudentName);
...
class Batch {
    ...
    void FillStudent(String RollNumber, String StudentName, int index){
        St[index] = new Student(RollNumber, StudentName);
    }
